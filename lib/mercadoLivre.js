...
async function getValidAccessToken(userId) {
  ...
  const idadeEmSegundos = (Date.now() - stored.saved_at) / 1000;

  const margemDeSeguranca = 60;
  const aindaValido =
    idadeEmSegundos < stored.expires_in - margemDeSeguranca;

  if (aindaValido) {
    return stored.access_token;
  }

  const novosTokens = await refreshAccessToken(stored.refresh_token);

  await saveTokens(userId, novosTokens);
